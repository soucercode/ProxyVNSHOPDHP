                ...
                Text(busyIndex == index
                     ...
                     : (enabled.contains(index)
                        ...
                        : (failedIndices.contains(index)
                           ...
                           : feature.description)))
                    .font(.caption2)
                    .foregroundStyle(enabled.contains(index) ? Color.green : (failedIndices.contains(index) ? Color.red : Color.secondary))
            }
            ...
            do {
                if value {
                    guard let resourceName = feature.patchResource else {
                        // UI-only feature: keep the same loading interaction even when
                        // there is no bundled private-app asset for this feature.
                        try await Task.sleep(nanoseconds: 4_000_000_000)
                        enabled.insert(index)
                        failedIndices.remove(index)
                        state.showToast("Đã bật \(feature.name) ✓")
                        return
                    }
