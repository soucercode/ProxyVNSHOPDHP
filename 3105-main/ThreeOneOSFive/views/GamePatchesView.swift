...
struct GamePatchesView: View {
    ...
    private func isAvailable(_ feature: LocalPatchFeature) -> Bool {
        guard let localGame,
            ...
            // Features without a definition (e.g. Aim Drag / Location) remain unavailable.
            return false
        }
    ...
    private func setFeature(_ feature: LocalPatchFeature, _ value: Bool) {
        guard let localGame,
              let definition = LocalPatchDefinitions.definition(for: feature, game: localGame),
              let _ = PatchAssetLoader.url(for: definition)
        else {
            return
        }
        ...
        Task {
            ...
            if value {
                do {
                    ...
                    toast = ToastMessage(text: "\(feature.rawValue) đã sẵn sàng")
                } catch {
                    enabled[feature] = false
                    toast = ToastMessage(text: "Không thể áp dụng bản thử nghiệm")
                }
            } else {
                enabled[feature] = false
            }
